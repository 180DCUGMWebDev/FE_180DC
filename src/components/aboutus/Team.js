...
export default function Team() {
  ...
  const [data, setData] = useState({ term: terms[0], open: false });
  ...
  const handleChangeTerm = async (item) => {
    if (hydrate) {
      setData((prevData) => ({
        term: item,
        open: prevData.term === item ? !prevData.open : true, // Correctly toggling `open` based on the previous state
      }));
    }
  };
            ...
            {hydrate && (
              <div className="mb-[24px] flex w-full flex-col items-center justify-center text-center">
                ...
                >
                  ...
                  <IoTriangle
                    className={`w-[8%] duration-500 ${data.open ? "rotate-0" : "rotate-180"}`}
                  />
                  <div
                    className={`${data.open ? "flex -translate-y-0 flex-col opacity-100" : "z-[-1] -translate-y-[100%] bg-[red] opacity-0"} absolute left-0 top-[101%] z-[12] h-fit w-full gap-0 rounded-b-lg border-[2px] border-[#58B9D1] py-[1%] transition-all duration-500 md:border-[3px]`}
                  >
                    {terms.map((item) => (
                      <button
                        key={item}
