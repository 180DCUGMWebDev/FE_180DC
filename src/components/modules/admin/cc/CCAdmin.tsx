...
export function CCAdmin({
  submissions: initialSubmissions,
  adminUser,
}: {
  submissions: CCSubmission[];
  adminUser: any;
}) {
  ...
  async function handleAccept(submission: CCSubmission) {
    setProcessingId(submission.id);
    setAcceptModalOpen(false);
    try {
      const response = await fetch("/api/casecomp/teams/accept", {
        method: "POST",
  ...
  function openRejectModal(submission: CCSubmission) {
    setSelectedSubmission(submission);
  ...
  async function handleReject(reason: string) {
    ...
    try {
      const response = await fetch("/api/casecomp/teams/reject", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
  ...
  async function handleBroadcast(targetGroup: string, subject: string, content: string) {
    ...
    try {
      const res = await fetch("/api/casecomp/teams/broadcast", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  function exportToCSV() {
    ...
    a.href = url;
    a.download = `casecomp-registrations-${new Date().toISOString().split("T")[0]}.csv`;
    a.click();
          ...
          <div className="flex items-center justify-between">
            <div>
              <h1 className="font-avenir-black text-3xl text-gray-900">
                180DC Case Competition - Admin Dashboard
              </h1>
                ...
                <div className="border-t border-gray-200 pt-4">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {[
                      ...
                      { key: "doc_cv", label: "CV", value: s.doc_cv },
                      { key: "doc_follow", label: "Follow Proof", value: s.doc_follow },
                      { key: "doc_twibbon", label: "Twibbon Proof", value: s.doc_twibbon },
                      {
