  ...
  return (
    <div className="flex w-full flex-col gap-4 rounded-xl border border-white/10 bg-[#333333] p-4 text-white md:flex-row md:items-center md:justify-between md:gap-0">
      ...
      <div className="flex w-full flex-row items-center gap-4 md:w-[35%] md:pl-2">
        <div
          className={`relative aspect-[4/3] w-[80px] shrink-0 overflow-hidden rounded-md ${
            item.type === "digital" ? "bg-white/90 p-1" : "bg-gray-500"
          } md:w-[100px]`}
        >
          <Image
            src={item.image}
            alt={item.name}
            fill
            className={item.type === "digital" ? "object-contain" : "object-cover"}
          />
        </div>
