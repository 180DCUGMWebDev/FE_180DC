...
export const insertRandom = async (
  ...
  const messageInfo = `Results for upserting '${randomString}' into '${config.table}' at column '${config.column}'`;
...
export const deleteRandom = async (
  ...
  const messageInfo = `Results for deleting '${entryToDelete}' from '${config.table}' at column '${config.column}'`;
...
export const determineAction = async (supabase: SupabaseClient): Promise<QueryResponse> => {
  ...
  if (retrievedEntries == null) {
    return {
      successful: false,
      message: `Received 'null' data when retrieving entries from ${config.table}\n${retrievalResults.message}`,
    };
  }
