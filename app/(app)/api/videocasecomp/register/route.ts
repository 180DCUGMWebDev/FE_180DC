...
import { driveFolderId, GetJWTAuth, GetOAuth, saveFileToDrive, sendEmail, uploadData } from "./utils";
...
const SHEET_HEADERS = [
  ...
  "Leader's Name", "Leader's University", "Leader's Major", "Leader's Batch", "Leader's Email", "Leader's Phone",
  "1st Member's Name", "1st Member's University", "1st Member's Major", "1st Member's Batch", "1st Member's Email", "1st Member's Phone",
  "2nd Member's Name", "2nd Member's University", "2nd Member's Major", "2nd Member's Batch", "2nd Member's Email", "2nd Member's Phone",
  "ID Card", "Follow", "Mention", "Repost", "Twibbon", "Bukti Pembayaran", "Rekening",
];
  ...
  try {
    ...
    const [idCardLink, followLink, mentionLink, repostLink, twibbonLink, buktiLink] =
      await Promise.all([
        ...
        doc.loadInfo(), // runs concurrently with uploads
      ]);
    ...
    await uploadData(
      ...
      teamLeader,
      teamMember,
      { idCard: idCardLink, follow: followLink, mention: mentionLink, repost: repostLink, twibbon: twibbonLink },
    ...
    await Promise.all([
      supabase.from("vcc-registrations").insert({
        status: "pending",
        payment,
